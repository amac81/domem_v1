...
<!--[if !IE]><!-->
<link href="<c:url value='/static/css/protocoltypesnoie.css'/>" rel="stylesheet">
<!--<![endif]-->
</head>
<c:if test="${fn:length(protocoltypes) == 0}">
	<h3 style="color: #ff0000";>Não existem tipos de protocolos na BD.</h3>
</c:if>
...
<c:set var="totalCount" scope="session" value="${fn:length(protocoltypes)}"/>
...
<c:if test="${totalCount > perPage}">
	<a href="javascript:loadToDiv('domotic/protocoltypes?start=0','Tipos de protocolos');">
		<img src='static/images/icons/first_page.png'></a>
	<a href="javascript:loadToDiv('domotic/protocoltypes?start=${pageStart - perPage}','Tipos de protocolos');">
	...
	<a href="javascript:loadToDiv('domotic/protocoltypes?start=${pageLast}','Tipos de protocolos');">
...
<table>
	...
	<c:forEach var="type" items="${protocoltypes}" begin="${pageStart}" end="${pageStart + perPage - 1}">
	    <tr>
			<td>${type.descricao}</td>
			<td style="text-align: center;">
 			<sec:authorize access="hasRole('ADMIN') or hasRole('SUPER')">
			 	<a href="javascript:deleteProtocolTypeModal('${type.descricao}','${type.id}');" class="btn icon-btn btn-danger" >
					<span style="color: #ffffff;" class="glyphicon btn-glyphicon glyphicon-trash img-circle text-danger"></span>
				</a>
				<a class="btn icon-btn btn-info" href="javascript:editProtocolTypeModal('${type.descricao}','${type.id}');">
...
<!-- MODAL EDIT MODAL EDIT PROTOCOLTYPE -->
<div class="modal fade" id="edit-protocoltype-modal" tabindex="-1" role="dialog"
...
<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body" id="modalEditProtocolTypeDivBody">
			<!-- Form editar -->
			<!-- /Form editar -->					
			</div>
		</div>
...
<!-- /MODAL EDIT PROTOCOLTYPE -->


<!-- MODAL DELETE PROTOCOLTYPE -->
<div class="modal fade" id="confirm-protocoltype-delete" tabindex="-1" role="dialog"
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<a href="javascript:confirmProtocolTypeDelete('${_csrf.token}');" class="btn btn-danger">Confirmar</a>
			</div>
		</div>
	</div>
</div>
...
<sec:authorize access="hasRole('ADMIN') or hasRole('SUPER')">
	<a href="javascript:novoProtocoltypeModal();" class="btn btn-primary">Adicionar novo</a> 
</sec:authorize>
