...
var openWsocketPilight = false;

//Create stomp client over sockJS protocol
var socket = new SockJS("/domem/wsdomem");
...
var connectCallback = function() {
	stompClient.subscribe('/domemwsmsgbroker/msg', msgReceived);
		
	stompClient.send('/apipilight/requestconfig');

	openWsocketPilight = true;
	escreveInfo('DOMEM STOMP SOCKET: ligação com sucesso!');	
};

var stompFailureCallback = function (error) {
	openWsocketPilight = false;
	escreveInfo('DOMEM STOMP SOCKET: ' + error);

    setTimeout(stompConnect, 5000);
    //page reload
    location.reload();
	escreveInfo('DOMEM STOMP SOCKET: A religar em 5 segundos.');	
};

function stompConnect() {
	escreveInfo('DOMEM STOMP SOCKET: Tentativa de ligação.');
...
function msgReceived(msg) {	
	...
	var data = $.parseJSON(wSocketMsg);
	
	if('pilightoff' in data) {
		escreveInfo("<img style='display:block; margin-left:auto; margin-right:auto;"+
					"width: 150px; height: 150px;' src='static/images/warning.png'>"+
				 	"<h4 style='color: #3d8f8f; text-align: center;'>Não é possível ligar ao Pilight!</h4>"+
				 	"<h5 style='color: #3d8f8f; text-align: center;'>Tente novamente. </h5>");
	}
	else{
		escondeInfo();
...
var language_pt = {
	...
	connecting: "A ligar",
	connection_lost: "Ligação perdida, click para religar",
	connection_failed: "Falhou ao ligar, click para recarregar",
...
function toggleTabs() {
	...
	if(openWsocketPilight) {
		stompClient.send("/apipilight/sendcommand", {}, json);
	}
	...
	$.each(data['gui'], function(dindex, dvalues) {
		...
		if($('#'+alphaNum(lindex)).length == 0) {				
			if(bShowTabs) {
				...
				oNavBar.navbar();
				$('#content').append($('<div class="content" id="'+alphaNum(lindex)+'"><ul data-role="listview" data-inset="true" data-theme="b"></ul></div>'));
		...
		if(!bShowTabs) {
			if($('#heading_'+alphaNum(lindex)).length == 0) {
				if($('#all').length == 0) {
					$('#content').append($('<ul id="all" data-role="listview" data-inset="true" data-theme="b"><li data-role="list-divider" id="heading_'+alphaNum(lindex)+'" class="heading" role="heading">'+lindex+'</li></ul>'));
				} else {
					...
					if(vindex == 'temperature' && $('#'+dvalues+'_temp').length > 0) {
						...
						if(vvalues == 'rise') {
							...
							if($('#'+dvalues+'_sunset_icon').attr("class").indexOf("gray") == -1) {
								$('#'+dvalues+'_sunset_icon').removeClass('green').addClass('gray');
							}
						} else {
							if($('#'+dvalues+'_sunrise_icon').attr("class").indexOf("gray") == -1) {
								$('#'+dvalues+'_sunrise_icon').removeClass('yellow').addClass('gray');
							}
							if($('#'+dvalues+'_sunset_icon').attr("class").indexOf("green") == -1) {
								$('#'+dvalues+'_sunset_icon').removeClass('gray').addClass('green');
							}
						}
...
function parseData(data) {
	if(data.hasOwnProperty("gui") && data.hasOwnProperty("devices")) {
		...
		stompClient.send('/apipilight/requestvalues');
...
function escreveInfo(texto){
	document.getElementById('infodiv').innerHTML =  texto;
}

var timerTextShow = setInterval(function(){ 
    //esconde texto ao fim de 4 segundos.
	escondeInfo();
